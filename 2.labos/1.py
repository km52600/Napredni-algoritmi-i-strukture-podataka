...
def _rotate_and_update(tree: AVLTree, rotatee: AVLNode, rotator: AVLNode, rotation: RotationType) -> None:
    ...
    # TODO: Update heights where necessary
    if rotation=="Left" :
        _left_rotation_impl(rotatee,rotator)
    else :
        _right_rotation_impl(rotatee,rotator)

        rotatee.update_height
        rotator.update_height
        rotatee.left.update_height
        rotatee.right.update_height
        rotator.left.update_height
        rotator.right.update_height
...
def avl_balance(tree: AVLTree, node: Optional[AVLNode]) -> None:
    ...
    while node is not None:
        ...
        # TODO: Check if any balancing is required and call the _rotate_and_update function
        # where appropriate
        node = node.parent
        if balance_factor==2 :
            temp=node.right
            if temp.balance_factor()==0 or temp.balance_factor()==1 :
                _left_rotation_impl(node,temp)
            if temp.balance_factor()==-1 :
                temp2=temp.left
                _right_rotation_impl(temp,temp2)
                _left_rotation_impl(temp,node)

        if balance_factor==-2 :
            temp=node.left
            if temp.balance_factor()==0 or temp.balance_factor()==-1 :
                _right_rotation_impl(node,temp)
            if temp.balance_factor()==1 :
                temp2=temp.right
                _left_rotation_impl(temp,temp2)
                _right_rotation_impl(temp,node)
