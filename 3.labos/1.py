...
def red_uncle_case(
    ...
    parent: RBNodeBase,
    uncle: RBNodeBase,
    grandparent: Optional[RBNodeBase]
) -> None:
...
def broken_case(
    ...
    if node==parent.right and parent==grandparent.left and uncle.color==Color.Black:
        tree.rotate(parent,node,RotationType.Left)
        tree.insert_rebalance(parent,True)
    if node==parent.left and parent==grandparent.right and uncle.color==Color.Black:
