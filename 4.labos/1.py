...
def query_priority_subtree(tree: List[Node], index: int, limit_value: float) -> List[Point]:
    ...
    if node is not None and node.point[0]<=limit_value: # TODO: Replace the condition
...
def query(tree: List[Node], interval: Tuple[float, float], limit: float) -> List[Point]:
    ...
    while node is not None and node is not INF_NODE and \
        ...
        point = node.point
        if True: # TODO: Replace the condition
            result.append(point)
    ...
    node = get_node(tree, index)
    while node is not None and node.point[0]<=limit: # TODO: Replace the condition
        point = node.point
